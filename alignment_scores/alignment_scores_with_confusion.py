...
for counter_image in range(number_of_images):
    ...
    # print(shift_factor)
    # tensor_in_sa =  [0 for item in tensor_in_sa if item<0]
    # tensor_in_sa = [item + shift_factor for item in tensor_in_sa if item<0]
    # tensor_in_sa = numpy.reshape(tensor_in_sa,[res_source_t,res_source_h,res_source_w])
       
    #shifting ta
    # tensor_in_ta = numpy.reshape(tensor_in_ta,-1)
    # shift_factor = abs(numpy.min(tensor_in_ta))
    # print(shift_factor)
    # tensor_in_ta = [item + shift_factor for item in tensor_in_ta if item<0 ]
    # tensor_in_ta = numpy.reshape(tensor_in_ta,[res_source_t,res_source_h,res_source_w])
    ...
    #tensor_rand2 = numpy.random.rand(res_source_t,res_source_h,res_source_w)
        ...
        for counter_label in range(len(subinds)): 
            ...
            if subnoun in ref_names: 
                ...
                # very exceptionally nan might occure because of upsampling and thresholding of GT mask
                    ...
                    #cm_rand1[col,col] = cm_rand1[col,col] + scorerand1_sa
                    #cm_rand2[col,col] = cm_rand2[col,col] + scorerand2_sa
                    # confusion matrix 
                    ...
                    #score_r1_other, area_other = compute_spatial_alignment_other (t_onset_sa, t_offset_sa ,tensor_rand1, m_null_other)
                    #score_r2_other, area_other = compute_spatial_alignment_other (t_onset_sa, t_offset_sa ,tensor_rand2, m_null_other)
                    ...
                    if ~numpy.isnan(score_s_other):
                        ...
                        #cm_rand1[80,col] = cm_rand1[80,col] + score_r1_other
                        #cm_rand2[80,col] = cm_rand2[80,col] + score_r2_other
                else:
                    nanfound.append(counter_image)
                    
                    
                
scipy.io.savemat(path_out + file_out + '_T.mat' , {'all_sa_scores':all_sa_scores, 'all_ta_scores':all_ta_scores, 
                                                  ...
                                                  'cm_detection':cm_detection, 'cm_rand2':cm_rand2,' cm_object_area':cm_object_area})                
